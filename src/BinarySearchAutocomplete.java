...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		for (int i = first; i <= last; i++) {
			...
			if (pq.size() < k) {
				pq.add(x);
			} else if (pq.peek().getWeight() < x.getWeight()) {
				pq.remove();
				pq.add(x);
			}
		}
	...
	/**
	 * uses instance variable mySize and accounts for each double and
	 * however many characters are contained with in each term
	 * @return the amount of storage used by myTerms
	 */
	@Override
