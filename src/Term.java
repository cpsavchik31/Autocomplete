...
return myWord.compareTo(that.myWord);
	...
	public static class PrefixOrder implements Comparator<Term> {
		...
		public int compare(Term v, Term w) {
			int t = myPrefixSize;
			if (v.myWord.length() < this.myPrefixSize|| w.myWord.length() < myPrefixSize){
				t = Math.min(v.myWord.length(), w.myWord.length());
			}
			for (int i = 0; i < t; i++) {
				if(v.myWord.charAt(i) != w.myWord.charAt(i)){
					int x = Character.compare(v.myWord.charAt(i), w.myWord.charAt(i));
					return x;
				}
			}
			if (v.getWord().length() >= myPrefixSize && w.getWord().length() >= myPrefixSize) return 0;
